...
// Structure to represent a triple (operator, operand1, operand2)
typedef struct {
    char operator;
...
void generate_triple(char operator, Operand op1, Operand op2, Triple *triples, int *triple_count) {
    Triple triple;
    triple.operator = operator;
    if (op1.is_variable) {
        strcpy(triple.operand1, op1.name);
    } else {
        sprintf(triple.operand1, "%d", op1.value);
    }
    if (op2.is_variable) {
        strcpy(triple.operand2, op2.name);
    } else {
        sprintf(triple.operand2, "%d", op2.value);
    }

    // Add the triple to the list
    triples[*triple_count] = triple;
    (*triple_count)++;
}

// Function to print all triples
void print_triples(Triple *triples, int triple_count) {
    printf("\nTriples:\n");
    printf("Op\tOperand1\tOperand2\n");
...
int main() {
    ...
    Triple triples[MAX_EXPR_LENGTH]; // Array to store triples
    int triple_count = 0; // Counter for triples
    ...
    for (int i = 0; i < strlen(equals_sign); i++) {
        ...
        // If the character is a closing parenthesis, resolve the operations inside parentheses
            ...
            while (op_top >= 0 && op_stack[op_top] != '(') {
                ...
                generate_triple(op, op1, op2, triples, &triple_count);
    ...
    // Print all generated triples
    print_triples(triples, triple_count);
